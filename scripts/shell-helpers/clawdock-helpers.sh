#!/usr/bin/env bash
# ClawDock - Docker helpers for OpenNexus
# Inspired by Simon Willison's "Running OpenNexus in Docker"
# https://til.simonwillison.net/llms/opennexus-docker
#
# Installation:
...
# Common paths to check for OpenNexus
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/opennexus"
  "${HOME}/workspace/opennexus"
  "${HOME}/projects/opennexus"
  "${HOME}/dev/opennexus"
  "${HOME}/code/opennexus"
  "${HOME}/src/opennexus"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found OpenNexus at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/opennexus"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ OpenNexus not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/opennexus/opennexus.git ~/opennexus"
    echo "  cd ~/opennexus && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^OPENNEXUS_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d opennexus-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart opennexus-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f opennexus-gateway
}
...
clawdock-workspace() {
  cd ~/.opennexus/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec opennexus-gateway \
    bash -c 'echo "alias opennexus=\"./opennexus.mjs\"" > /tmp/.bashrc_opennexus && bash --rcfile /tmp/.bashrc_opennexus'
}
...
clawdock-cli() {
  _clawdock_compose run --rm opennexus-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build opennexus-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENNEXUS_GATEWAY_TOKEN=$token" opennexus-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" opennexus-gateway \
    bash -c './opennexus.mjs config set gateway.remote.token "$TOKEN" && ./opennexus.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec opennexus-gateway \
    bash -c "./opennexus.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart opennexus-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm opennexus-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec opennexus-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'opennexus config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for OpenNexus${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (opennexus alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to opennexus project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.opennexus)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'opennexus channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'opennexus status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.opennexus.ai${_CLR_RESET}"
  echo ""
}
