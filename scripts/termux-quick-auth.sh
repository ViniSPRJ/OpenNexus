...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/opennexus/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenNexus Re-Auth" -c "After getting key, run: ssh $SERVER '~/opennexus/scripts/mobile-reauth.sh'" --id opennexus-auth
        ;;
    *)
