...
# OpenNexus Auth Widget for Termux
...
SERVER="${OPENNEXUS_SERVER:-${CLAWDBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking OpenNexus auth..."

STATUS=$(ssh "$SERVER" '$HOME/opennexus/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/opennexus/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|OPENNEXUS_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/opennexus/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|OPENNEXUS_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: OpenNexus won't work until re-auth"
                ;;
        esac
        ;;
