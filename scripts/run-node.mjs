...
const shouldBuild = (deps) => {
  if (deps.env.OPENNEXUS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OPENNEXUS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[opennexus] ${message}\n`);
};

const runOpenNexus = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["opennexus.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runOpenNexus(deps);
  }
