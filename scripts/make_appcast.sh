...
ZIP=${1:?"Usage: $0 OpenNexus-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/opennexus/opennexus/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^OpenNexus-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/opennexus/opennexus/releases/download/v${VERSION}/"}
