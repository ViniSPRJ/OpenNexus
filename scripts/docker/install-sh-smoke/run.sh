...
INSTALL_URL="${OPENNEXUS_INSTALL_URL:-https://opennexus.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPENNEXUS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPENNEXUS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="opennexus"
PACKAGE_NAME="${OPENNEXUS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPENNEXUS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPENNEXUS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPENNEXUS_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
