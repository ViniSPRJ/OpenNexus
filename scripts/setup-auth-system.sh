#!/bin/bash
# Setup OpenNexus Auth Management System
...
echo "=== OpenNexus Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'opennexus-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenNexus message: Send warning via OpenNexus itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/opennexus-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/opennexus-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/opennexus-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now opennexus-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status opennexus-auth-monitor.timer"
echo ""
echo "Quick commands:"
