...
IMAGE_NAME="opennexus-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="opennexus-net-e2e-$$"
GW_NAME="opennexus-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OPENNEXUS_GATEWAY_TOKEN=$TOKEN" \
	  -e "OPENNEXUS_SKIP_CHANNELS=1" \
	  -e "OPENNEXUS_SKIP_GMAIL_WATCHER=1" \
	  -e "OPENNEXUS_SKIP_CRON=1" \
	  -e "OPENNEXUS_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
