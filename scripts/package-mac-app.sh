...
# Build and bundle OpenNexus into a minimal .app we can open.
# Outputs to dist/OpenNexus.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpenNexus.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OpenNexus"
BUNDLE_ID="${BUNDLE_ID:-ai.opennexus.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/opennexus/opennexus/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpenNexus/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpenNexusBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenNexusGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpenNexus"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpenNexus"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpenNexus"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpenNexus" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenNexus/Resources/OpenNexus.icns" "$APP_ROOT/Contents/Resources/OpenNexus.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpenNexus/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpenNexusKit resources"
OPENNEXUSKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenNexusKit_OpenNexusKit.bundle"
if [ -d "$OPENNEXUSKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenNexusKit_OpenNexusKit.bundle"
  cp -R "$OPENNEXUSKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenNexusKit_OpenNexusKit.bundle"
else
  echo "WARN: OpenNexusKit resource bundle not found at $OPENNEXUSKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpenNexus"
killall -q OpenNexus 2>/dev/null || true
