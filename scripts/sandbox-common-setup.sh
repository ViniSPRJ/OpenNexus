...
BASE_IMAGE="${BASE_IMAGE:-opennexus-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-opennexus-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=opennexus.sandbox=1)
NOTE
