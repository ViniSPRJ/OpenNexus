...
const unitIsolatedFilesRaw = [
  ...
  "src/agents/opennexus-tools.subagents.sessions-spawn.lifecycle.test.ts",
...
// OPENNEXUS_TEST_VM_FORKS=0, and let users force-enable with =1.
const supportsVmForks = Number.isFinite(nodeMajor) ? nodeMajor !== 24 : true;
const useVmForks =
  process.env.OPENNEXUS_TEST_VM_FORKS === "1" ||
  (process.env.OPENNEXUS_TEST_VM_FORKS !== "0" && !isWindows && supportsVmForks);
const disableIsolation = process.env.OPENNEXUS_TEST_NO_ISOLATE === "1";
const runs = [
  ...(useVmForks
    ? [
        {
...
const shardOverride = Number.parseInt(process.env.OPENNEXUS_TEST_SHARDS ?? "", 10);
...
const silentArgs =
  process.env.OPENNEXUS_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
const rawPassthroughArgs = process.argv.slice(2);
const passthroughArgs =
  rawPassthroughArgs[0] === "--" ? rawPassthroughArgs.slice(1) : rawPassthroughArgs;
const rawTestProfile = process.env.OPENNEXUS_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
  rawTestProfile === "max" ||
  ...
  rawTestProfile === "serial"
    ? rawTestProfile
    : "normal";
const overrideWorkers = Number.parseInt(process.env.OPENNEXUS_TEST_WORKERS ?? "", 10);
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.OPENNEXUS_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
...
function resolveReportDir() {
  const raw = process.env.OPENNEXUS_VITEST_REPORT_DIR?.trim();
  if (!raw) {
    return null;
  }
  try {
