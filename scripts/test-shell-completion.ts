 ...
 * `opennexus update`. Use it to verify the completion installation flow
...
\`opennexus update\`, \`opennexus doctor\`, and \`opennexus onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenNexus Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`opennexus completion --install\` later to enable.`));
    return;
  }
