...
#
# Suggested cron: */30 * * * * /home/admin/opennexus/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenNexus notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., opennexus-alerts)
...
STATE_FILE="$HOME/.opennexus/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenNexus if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use opennexus
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenNexus to $NOTIFY_PHONE..."
            opennexus send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenNexus is down. Run: ssh l36 '~/opennexus/scripts/mobile-reauth.sh'" "urgent"
    exit 1
