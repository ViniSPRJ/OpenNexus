#!/usr/bin/env bash
# Reset OpenNexus like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${OPENNEXUS_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="OpenNexus.app/Contents/MacOS/OpenNexus"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/OpenNexus"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/OpenNexus"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/OpenNexus"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.opennexus.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/opennexus-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${OPENNEXUS_RESTART_LOG:-/tmp/opennexus-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${OPENNEXUS_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.opennexus/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  OPENNEXUS_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node opennexus.mjs daemon install --force --runtime node"
      log "  node opennexus.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.opennexus/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.opennexus.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing OpenNexus instances"
kill_all_opennexus
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product OpenNexus"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/OpenNexus.app" ]]; then
    APP_BUNDLE="/Applications/OpenNexus.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/OpenNexus.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/OpenNexus.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/OpenNexus.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set OPENNEXUS_APP_BUNDLE to your installed OpenNexus.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node opennexus.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node opennexus.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".opennexus", "opennexus.json"), "utf8");
