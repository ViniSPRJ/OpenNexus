...
export function isCommandEnabled(cfg: OpenNexusConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenNexusConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenNexusConfig),
...
export function getCommandDetection(_cfg?: OpenNexusConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenNexusConfig) {
