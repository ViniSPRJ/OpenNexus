...
function createReplyConfig(home: string, streamMode?: "block"): OpenNexusConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("opennexus-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("OPENNEXUS_TEST_FAST", "1");
