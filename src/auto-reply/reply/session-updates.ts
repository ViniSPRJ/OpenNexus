...
export async function prependSystemEvents(params: {
  ...
  const resolveSystemEventTimezone = (cfg: OpenNexusConfig) => {
  ...
  const formatSystemEventTimestamp = (ts: number, cfg: OpenNexusConfig) => {
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.OPENNEXUS_TEST_FAST === "1") {
