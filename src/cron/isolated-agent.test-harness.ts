...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "opennexus-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".opennexus", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OpenNexusConfig> = {},
): OpenNexusConfig {
  const base: OpenNexusConfig = {
    agents: {
      defaults: {
