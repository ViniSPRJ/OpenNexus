...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".opennexus");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENNEXUS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENNEXUS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENNEXUS_STATE_DIR.
