...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredOpenNexusTmpDir: vi.fn(() => "/tmp/opennexus"),
}));
vi.mock("../infra/tmp-opennexus-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredOpenNexusTmpDir.mockReturnValue("/tmp/opennexus");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredOpenNexusTmpDir.mockReturnValue("/tmp/opennexus-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "opennexus-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredOpenNexusTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "opennexus-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "opennexus-preferred", "downloads")}${path.sep}`),
    );
  });
