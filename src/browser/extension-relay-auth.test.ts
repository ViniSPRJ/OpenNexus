...
import {
  probeAuthenticatedOpenNexusRelay,
...
describe("extension-relay-auth", () => {
  ...
  beforeEach(() => {
    prevGatewayToken = process.env.OPENNEXUS_GATEWAY_TOKEN;
  ...
  it("accepts authenticated opennexus relay probe responses", async () => {
    ...
    await withRelayServer(
      ...
      async ({ port }) => {
        ...
        const ok = await probeAuthenticatedOpenNexusRelay({
          baseUrl: `http://127.0.0.1:${port}`,
          relayAuthHeader: "x-opennexus-relay-token",
