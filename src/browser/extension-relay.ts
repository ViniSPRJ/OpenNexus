...
const RELAY_AUTH_HEADER = "x-opennexus-relay-token";
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/OpenNexus-Extension-Relay",
          revision: "0",
          userAgent: "OpenNexus-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "OpenNexus/extension-relay",
  ...
  try {
    ...
    if (
      ...
      (await probeAuthenticatedOpenNexusRelay({
