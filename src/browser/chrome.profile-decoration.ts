...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".opennexus-profile-decorated");
}
 ...
 */
export function decorateOpenNexusProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENNEXUS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENNEXUS_BROWSER_COLOR).toUpperCase();
