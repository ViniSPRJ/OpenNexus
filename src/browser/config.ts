import type { BrowserConfig, BrowserProfileConfig, OpenNexusConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENNEXUS_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "opennexus" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_OPENNEXUS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "opennexus" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENNEXUS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENNEXUS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenNexus browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for opennexus/opennexus CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenNexusConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENNEXUS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "opennexus";
