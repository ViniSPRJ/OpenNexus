...
const RELAY_TOKEN_CONTEXT = "opennexus-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const OPENNEXUS_RELAY_BROWSER = "OpenNexus/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.OPENNEXUS_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or OPENNEXUS_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedOpenNexusRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === OPENNEXUS_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
