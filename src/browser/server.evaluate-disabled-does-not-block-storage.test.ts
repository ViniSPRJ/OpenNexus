  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          opennexus: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.OPENNEXUS_GATEWAY_PORT;
    process.env.OPENNEXUS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.OPENNEXUS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.OPENNEXUS_GATEWAY_PASSWORD;
