...
import { loadOpenNexusPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `opennexus-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.OPENNEXUS_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "opennexus.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.OPENNEXUS_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadOpenNexusPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadOpenNexusPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.OPENNEXUS_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.OPENNEXUS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOpenNexusPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadOpenNexusPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOpenNexusPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.OPENNEXUS_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ OPENNEXUS_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
