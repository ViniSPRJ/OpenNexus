...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `opennexus-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "opennexus.plugin.json"), JSON.stringify(manifest), "utf-8");
}
