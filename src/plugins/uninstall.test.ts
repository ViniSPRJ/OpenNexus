...
function createSinglePluginWithEmptySlotsConfig(): OpenNexusConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): OpenNexusConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "opennexus-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-opennexus-extensions/my-plugin",
      },
      extensionsDir,
    });
