...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "opennexus-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "opennexus-bundled-skills-test-"));
    process.env.OPENNEXUS_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
