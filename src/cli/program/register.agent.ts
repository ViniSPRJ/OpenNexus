...
${formatHelpExamples([
  ['opennexus agent --to +15555550123 --message "status update"', "Start a new session."],
  ['opennexus agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'opennexus agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'opennexus agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['opennexus agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'opennexus agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.opennexus.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.opennexus.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['opennexus agents set-identity --agent main --name "OpenNexus" --emoji "🦞"', "Set name + emoji."],
  ["opennexus agents set-identity --agent main --avatar avatars/opennexus.png", "Set avatar path."],
  [
    "opennexus agents set-identity --workspace ~/.opennexus/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "opennexus agents set-identity --identity-file ~/.opennexus/workspace/IDENTITY.md --agent main",
