...
beforeEach(() => {
  ...
  originalHideBanner = process.env.OPENNEXUS_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.OPENNEXUS_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.OPENNEXUS_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("opennexus");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "opennexus", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("opennexus-status");
  });

  it("loads plugin registry for plugin-required commands", async () => {
    await runCommand({
      parseArgv: ["message", "send"],
      processArgv: ["node", "opennexus", "message", "send"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "opennexus", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "opennexus", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "opennexus", "--version"],
    });
  ...
  it("hides banner when OPENNEXUS_HIDE_BANNER is truthy", async () => {
    process.env.OPENNEXUS_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "opennexus", "status"],
    });
