    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opennexus nodes status", "List known nodes with live status."],
          ["opennexus nodes pairing pending", "Show pending node pairing requests."],
          ['opennexus nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["opennexus nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.opennexus.ai/cli/nodes")}\n`,
    );
