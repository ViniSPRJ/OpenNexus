    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opennexus memory status", "Show index and provider status."],
          ["opennexus memory index --force", "Force a full reindex."],
          ['opennexus memory search --query "deployment notes"', "Search indexed memory entries."],
          ["opennexus memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.opennexus.ai/cli/memory")}\n`,
    );
