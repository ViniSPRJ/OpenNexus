...
import type { OpenNexusConfig, ConfigFileSnapshot } from "../config/types.opennexus.js";
...
vi.mock("../infra/opennexus-root.js", () => ({
  resolveOpenNexusPackageRoot: vi.fn(),
}));
...
const { resolveOpenNexusPackageRoot } = await import("../infra/opennexus-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/opennexus-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as OpenNexusConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/opennexus-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOpenNexusPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("opennexus-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveOpenNexusPackageRoot).mockClear();
    ...
    vi.mocked(resolveOpenNexusPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("opennexus");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/opennexus-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "opennexus-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("OpenNexus update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as OpenNexusConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveOpenNexusPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("opennexus-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ OPENNEXUS_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.OPENNEXUS_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("opennexus-update-wizard");
    await withEnvAsync({ OPENNEXUS_GIT_DIR: tempDir }, async () => {
      setTty(true);
