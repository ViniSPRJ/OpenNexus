    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "opennexus node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["opennexus node status", "Check node host service status."],
          ["opennexus node install", "Install the node host service."],
          ["opennexus node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.opennexus.ai/cli/node")}\n`,
    );
