...
import type { ConfigFileSnapshot, OpenNexusConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `opennexus config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: OpenNexusConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: OpenNexusConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: OpenNexusConfig, config: OpenNexusConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: OpenNexusConfig = {
        agents: {
      ...
      const runtimeMerged: OpenNexusConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
