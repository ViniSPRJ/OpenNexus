...
import { resolveOpenNexusPackageRoot } from "../../infra/opennexus-root.js";
...
const OPENNEXUS_REPO_URL = "https://github.com/opennexus/opennexus.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "opennexus";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("opennexus@")) {
    return trimmed.slice("opennexus@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.OPENNEXUS_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveOpenNexusPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OPENNEXUS_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENNEXUS_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENNEXUS_GIT_DIR to an empty folder or an opennexus checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPENNEXUS_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "opennexus.mjs");
