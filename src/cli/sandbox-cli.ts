...
const SANDBOX_EXAMPLES = {
  main: [
    ["opennexus sandbox list", "List all sandbox containers."],
    ["opennexus sandbox list --browser", "List only browser containers."],
    ["opennexus sandbox recreate --all", "Recreate all containers."],
    ["opennexus sandbox recreate --session main", "Recreate a specific session."],
    ["opennexus sandbox recreate --agent mybot", "Recreate agent containers."],
    ["opennexus sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["opennexus sandbox list", "List all sandbox containers."],
    ["opennexus sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["opennexus sandbox recreate --all", "Recreate all containers."],
    ["opennexus sandbox recreate --session main", "Recreate a specific session."],
    ["opennexus sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["opennexus sandbox recreate --browser --all", "Recreate only browser containers."],
    ["opennexus sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["opennexus sandbox explain", "Show effective sandbox config."],
    ["opennexus sandbox explain --session agent:main:main", "Explain a specific session."],
    ["opennexus sandbox explain --agent work", "Explain an agent sandbox."],
    ["opennexus sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.opennexus.ai/cli/sandbox")}\n`,
    )
    .action(() => {
