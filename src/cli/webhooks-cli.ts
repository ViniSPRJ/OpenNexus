...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.opennexus.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + OpenNexus hooks")
    ...
    .option("--hook-url <url>", "OpenNexus hook URL")
    .option("--hook-token <token>", "OpenNexus hook token")
