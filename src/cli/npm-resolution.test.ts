...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@opennexus/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@opennexus/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@opennexus/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@opennexus/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @opennexus/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@opennexus/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@opennexus/plugin-alpha@1.2.3",
        installPath: "/tmp/opennexus/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@opennexus/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @opennexus/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@opennexus/plugin-alpha@latest",
      true,
      "/tmp/opennexus/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@opennexus/plugin-alpha@latest",
      installPath: "/tmp/opennexus/extensions/alpha",
      version: "1.2.3",
