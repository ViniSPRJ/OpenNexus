      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["opennexus gateway run", "Run the gateway in the foreground."],
            ["opennexus gateway status", "Show service status and probe reachability."],
            ["opennexus gateway discover", "Find local and wide-area gateway beacons."],
            ["opennexus gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.opennexus.ai/cli/gateway")}\n`,
      ),
  );
