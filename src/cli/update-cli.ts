...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OpenNexus and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["opennexus update", "Update a source checkout (git)"],
        ["opennexus update --channel beta", "Switch to beta channel (git + npm)"],
        ["opennexus update --channel dev", "Switch to dev channel (git + npm)"],
        ["opennexus update --tag beta", "One-off update to a dist-tag or version"],
        ["opennexus update --dry-run", "Preview actions without changing anything"],
        ["opennexus update --no-restart", "Update without restarting the service"],
        ["opennexus update --json", "Output result as JSON"],
        ["opennexus update --yes", "Non-interactive (accept downgrade prompts)"],
        ["opennexus update wizard", "Interactive update wizard"],
        ["opennexus --update", "Shorthand for opennexus update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run opennexus update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.opennexus.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opennexus update status", "Show channel + version status."],
          ["opennexus update status --json", "JSON output."],
          ["opennexus update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.opennexus.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
