...
import { resolveOpenNexusAgentDir } from "../agents/agent-paths.js";
...
import { ensureOpenNexusModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, OpenNexusConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.OPENNEXUS_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.OPENNEXUS_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.OPENNEXUS_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: OpenNexusConfig;
  agentDir: string;
}): OpenNexusConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<OpenNexusConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.OPENNEXUS_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.OPENNEXUS_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveOpenNexusAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-live-state-"));
  process.env.OPENNEXUS_STATE_DIR = tempStateDir;
  ...
  process.env.OPENNEXUS_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.opennexus-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveOpenNexusAgentDir();
  const sanitizedCfg: OpenNexusConfig = {
    ...params.cfg,
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "OpenNexus live tool probe (local, safe): " +
    ...
    process.env.OPENNEXUS_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.OPENNEXUS_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureOpenNexusModelsJson(cfg);
      ...
      const rawModels = process.env.OPENNEXUS_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.opennexus-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
