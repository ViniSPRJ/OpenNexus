...
type GetReplyFromConfigFn = (
  ...
  configOverride?: OpenNexusConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `opennexus-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.OPENNEXUS_CONFIG_PATH = path.join(root, "opennexus.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "opennexus.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "opennexus-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadOpenNexusPlugins: () => pluginRegistryState.registry,
  };
});
