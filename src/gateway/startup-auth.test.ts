...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: OpenNexusConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: OpenNexusConfig) {
  ...
  async function expectNoTokenGeneration(cfg: OpenNexusConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          OPENNEXUS_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
