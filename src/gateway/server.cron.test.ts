...
async function cleanupCronTestRun(params: {
  ...
  process.env.OPENNEXUS_SKIP_CRON = params.prevSkipCron;
}
...
async function setupCronTestRun(params: {
  ...
  const prevSkipCron = process.env.OPENNEXUS_SKIP_CRON;
  process.env.OPENNEXUS_SKIP_CRON = "0";
...
describe("gateway server cron", () => {
  ...
  test("writes cron run history and auto-runs due jobs", async () => {
    const { prevSkipCron, dir } = await setupCronTestRun({
      tempPrefix: "opennexus-gw-cron-log-",
    });
  ...
  test("posts webhooks for delivery mode and legacy notify fallback only when summary exists", async () => {
    ...
    const { prevSkipCron, dir } = await setupCronTestRun({
      tempPrefix: "opennexus-gw-cron-webhook-",
      cronEnabled: false,
