...
import type { OpenNexusConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: OpenNexusConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: OpenNexusConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: OpenNexusConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: OpenNexusConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: OpenNexusConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: OpenNexusConfig) => {
