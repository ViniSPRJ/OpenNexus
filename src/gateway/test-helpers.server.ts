...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "OPENNEXUS_BUNDLED_PLUGINS_DIR",
  "OPENNEXUS_SKIP_CHANNELS",
  "OPENNEXUS_SKIP_PROVIDERS",
  "OPENNEXUS_SKIP_CRON",
  "OPENNEXUS_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.OPENNEXUS_SKIP_PROVIDERS = "1";
  process.env.OPENNEXUS_SKIP_CRON = "1";
  process.env.OPENNEXUS_TEST_MINIMAL_GATEWAY = "1";
  process.env.OPENNEXUS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "opennexus-test-no-bundled-extensions")
    : "opennexus-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "opennexus-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".opennexus-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__opennexusTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__opennexusTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["OPENNEXUS_GATEWAY_TOKEN"]);
  const prev = process.env.OPENNEXUS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENNEXUS_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.OPENNEXUS_GATEWAY_PASSWORD;
