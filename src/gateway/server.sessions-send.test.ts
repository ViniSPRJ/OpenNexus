...
const { createOpenNexusTools } = await import("../agents/opennexus-tools.js");
...
type SessionSendTool = ReturnType<typeof createOpenNexusTools>[number];

function getSessionsSendTool(): SessionSendTool {
  const tool = createOpenNexusTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["OPENNEXUS_GATEWAY_PORT", "OPENNEXUS_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.OPENNEXUS_GATEWAY_PORT = String(gatewayPort);
  process.env.OPENNEXUS_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    const configPath = process.env.OPENNEXUS_CONFIG_PATH;
    if (!configPath) {
      throw new Error("OPENNEXUS_CONFIG_PATH missing in gateway test environment");
    }
