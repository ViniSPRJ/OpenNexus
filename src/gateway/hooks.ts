...
export function resolveHooksConfig(cfg: OpenNexusConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OpenNexusConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-opennexus-token"] === "string"
      ? req.headers["x-opennexus-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
