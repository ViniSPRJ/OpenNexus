...
import type { OpenNexusConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredOpenNexusTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "opennexus.log"); // legacy single-file path

const LOG_PREFIX = "opennexus";
...
function resolveSettings(): ResolvedSettings {
  let cfg: OpenNexusConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => OpenNexusConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.OPENNEXUS_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[opennexus] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
