...
describe("resolveGatewayConnection", () => {
  ...
  it("uses OPENNEXUS_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ OPENNEXUS_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers OPENNEXUS_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ OPENNEXUS_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
