...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENNEXUS_GATEWAY_PORT).toBe("18789");
    expect(env.OPENNEXUS_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENNEXUS_SERVICE_MARKER).toBe("opennexus");
    expect(env.OPENNEXUS_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENNEXUS_SERVICE_VERSION).toBe("string");
    expect(env.OPENNEXUS_SYSTEMD_UNIT).toBe("opennexus-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENNEXUS_LAUNCHD_LABEL).toBe("ai.opennexus.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENNEXUS_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENNEXUS_SYSTEMD_UNIT).toBe("opennexus-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENNEXUS_LAUNCHD_LABEL).toBe("ai.opennexus.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".opennexus"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPENNEXUS_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".opennexus-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", OPENNEXUS_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".opennexus"));
  });

  it("uses OPENNEXUS_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OPENNEXUS_STATE_DIR: "/var/lib/opennexus" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/opennexus"));
  });

  it("expands ~ in OPENNEXUS_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OPENNEXUS_STATE_DIR: "~/opennexus-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/opennexus-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OPENNEXUS_STATE_DIR: "C:\\State\\opennexus" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\opennexus");
  });
});
