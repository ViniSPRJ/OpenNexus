...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.OPENNEXUS_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service OPENNEXUS_GATEWAY_TOKEN does not match gateway.auth.token in opennexus.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `opennexus gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
