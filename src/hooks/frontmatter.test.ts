...
`;
    ...
    expect(parsed.opennexus.emoji).toBe("💾");
    expect(parsed.opennexus.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.opennexus.emoji).toBe("📝");
    expect(parsed.opennexus.events).toEqual(["command"]);
    expect(parsed.opennexus.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.opennexus.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"opennexus": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"opennexus": {"events": ["test"]}}');
  });
...
describe("resolveOpenNexusMetadata", () => {
  it("extracts opennexus metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenNexusMetadata(frontmatter);
  ...
  it("returns undefined when opennexus key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        opennexus: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenNexus" },
            { id: "npm", kind: "npm", package: "@opennexus/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@opennexus/hook");
  });
...
`;
    ...
    const opennexus = resolveOpenNexusMetadata(frontmatter);
    expect(opennexus).toBeDefined();
    expect(opennexus?.emoji).toBe("💾");
    expect(opennexus?.events).toEqual(["command:new", "command:reset"]);
    expect(opennexus?.requires?.config).toEqual(["workspace.dir"]);
    expect(opennexus?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const opennexus = resolveOpenNexusMetadata(frontmatter);
    expect(opennexus?.emoji).toBe("disk");
    expect(opennexus?.events).toEqual(["command:new"]);
  });
});
