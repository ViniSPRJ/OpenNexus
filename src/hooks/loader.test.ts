...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["OPENNEXUS_BUNDLED_HOOKS_DIR"]);
    process.env.OPENNEXUS_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
