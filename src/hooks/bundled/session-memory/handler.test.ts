...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => OpenNexusConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("opennexus-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): OpenNexusConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
