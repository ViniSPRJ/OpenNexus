...
This directory contains hooks that ship with OpenNexus. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.opennexus/opennexus.json`:
...
2. Restart gateway: `pkill -9 -f 'opennexus.*gateway' && pnpm opennexus gateway`
3. Enable the hook: `opennexus hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.opennexus.ai/automation/hooks
