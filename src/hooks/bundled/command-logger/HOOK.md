...
homepage: https://docs.opennexus.ai/automation/hooks#command-logger
metadata:
  {
    "opennexus":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.opennexus/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.opennexus/logs/commands.log ~/.opennexus/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/opennexus`:
   ```
   /home/username/.opennexus/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
