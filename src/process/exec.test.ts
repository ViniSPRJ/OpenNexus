...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ OPENNEXUS_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.OPENNEXUS_BASE_ENV ?? "") + "|" + (process.env.OPENNEXUS_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { OPENNEXUS_TEST_ENV: "ok" },
        },
      );
