...
describe("shared/frontmatter", () => {
  ...
  test("resolveOpenNexusManifestBlock parses JSON5 metadata and picks opennexus block", () => {
    const frontmatter = {
      metadata: "{ opennexus: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveOpenNexusManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveOpenNexusManifestBlock returns undefined for invalid input", () => {
    expect(resolveOpenNexusManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveOpenNexusManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveOpenNexusManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
