...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("opennexus-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.OPENNEXUS_STATE_DIR).toBe(path.join(tempHome.home, ".opennexus"));
    ...
    expect(process.env.OPENNEXUS_STATE_DIR).toBe(previousStateDir);
