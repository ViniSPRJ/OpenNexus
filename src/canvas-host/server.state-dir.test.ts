...
describe("canvas host state dir defaults", () => {
  it("uses OPENNEXUS_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("opennexus-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("OpenNexus Canvas");
      } finally {
        await handler.close();
      }
    });
