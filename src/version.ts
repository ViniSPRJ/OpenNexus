...
declare const __OPENNEXUS_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "opennexus";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["OPENNEXUS_VERSION"],
      env["OPENNEXUS_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current OpenNexus version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __OPENNEXUS_VERSION__ === "string" && __OPENNEXUS_VERSION__) ||
  process.env.OPENNEXUS_BUNDLED_VERSION ||
