...
const TEMP_MEDIA_PREFIX = "opennexus-media-";
...
function createGroqAudioConfig(): OpenNexusConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): OpenNexusConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): OpenNexusConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: OpenNexusConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredOpenNexusTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        OPENNEXUS_AGENT_DIR: isolatedAgentDir,
