...
function telegramCfg(): OpenNexusConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OpenNexusConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: OpenNexusConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as OpenNexusConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as OpenNexusConfig;
...
function createSignalAccountOverrideCfg(): OpenNexusConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as OpenNexusConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as OpenNexusConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as OpenNexusConfig,
        accountId: undefined,
        params: {
