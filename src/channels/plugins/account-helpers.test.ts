...
function cfg(accounts?: Record<string, unknown> | null): OpenNexusConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as OpenNexusConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as OpenNexusConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as OpenNexusConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as OpenNexusConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as OpenNexusConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as OpenNexusConfig)).toBe("default");
    });
  });
});
