...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@opennexus/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "opennexus-catalog-"));
        ...
        entries: [
          {
            ...
            opennexus: {
              ...
              install: {
                npmSpec: "@opennexus/demo-channel",
              },
            },
          },
        ],
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: OpenNexusConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
