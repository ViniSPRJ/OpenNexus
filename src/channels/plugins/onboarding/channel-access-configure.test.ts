...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: OpenNexusConfig, policy: ChannelAccessPolicy) => OpenNexusConfig;
  resolveAllowlist: (params: { cfg: OpenNexusConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OpenNexusConfig; resolved: TResolved }) => OpenNexusConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OpenNexusConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OpenNexusConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: OpenNexusConfig, policy: ChannelAccessPolicy): OpenNexusConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: OpenNexusConfig, policy: ChannelAccessPolicy): OpenNexusConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: OpenNexusConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: OpenNexusConfig; resolved: string[] }) => {
      calls.push("apply");
