...
import { discoverOpenNexusPlugins } from "../../plugins/discovery.js";
import type { OpenNexusPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, OpenNexusPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["OPENNEXUS_PLUGIN_CATALOG_PATHS", "OPENNEXUS_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<OpenNexusPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: OpenNexusPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: OpenNexusPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverOpenNexusPlugins({ workspaceDir: options.workspaceDir });
