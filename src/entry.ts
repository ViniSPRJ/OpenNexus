...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "opennexus.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "opennexus.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.OPENNEXUS_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.OPENNEXUS_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.OPENNEXUS_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[opennexus] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[opennexus] ${parsed.error}`);
      process.exit(2);
    }
