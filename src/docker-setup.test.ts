...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "opennexus-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    OPENNEXUS_GATEWAY_TOKEN: "test-token",
    OPENNEXUS_CONFIG_DIR: join(sandbox.rootDir, "config"),
    OPENNEXUS_WORKSPACE_DIR: join(sandbox.rootDir, "opennexus"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENNEXUS_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      OPENNEXUS_EXTRA_MOUNTS: undefined,
      OPENNEXUS_HOME_VOLUME: "opennexus-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENNEXUS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("OPENNEXUS_EXTRA_MOUNTS=");
    expect(envFile).toContain("OPENNEXUS_HOME_VOLUME=opennexus-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("opennexus-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("opennexus-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg OPENNEXUS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENNEXUS_CONFIG_DIR: configDir,
      OPENNEXUS_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("rejects injected multiline OPENNEXUS_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENNEXUS_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OPENNEXUS_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid OPENNEXUS_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENNEXUS_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid OPENNEXUS_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENNEXUS_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OPENNEXUS_HOME_VOLUME must match");
  });
