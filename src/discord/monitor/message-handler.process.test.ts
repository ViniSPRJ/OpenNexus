...
const resolveStorePath = vi.fn(() => "/tmp/opennexus-discord-process-test-sessions.json");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/opennexus-discord-process-test-sessions.json");
  ...
  async function createBlockModeContext() {
    return await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/opennexus-discord-process-test-sessions.json" },
        channels: {
          discord: {
