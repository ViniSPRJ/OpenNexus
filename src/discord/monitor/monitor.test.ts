...
describe("agent components", () => {
  const createCfg = (): OpenNexusConfig => ({}) as OpenNexusConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): OpenNexusConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/opennexus-sessions-test.json");
  });
