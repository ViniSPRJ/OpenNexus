...
function resolveVoiceTtsConfig(params: { cfg: OpenNexusConfig; override?: TtsConfig }): {
  cfg: OpenNexusConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredOpenNexusTmpDir(), "discord-voice-"));
