...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "opennexus.sandbox=1");
  args.push("--label", `opennexus.sessionKey=${params.scopeKey}`);
  args.push("--label", `opennexus.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `opennexus.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "opennexus.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`opennexus sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`opennexus sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("opennexus sandbox recreate --all");
}
