...
import { createOpenNexusCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: OpenNexusConfig) {
    return createOpenNexusCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<OpenNexusConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<OpenNexusConfig["agents"]>["list"]>[number]["tools"];
  }): OpenNexusConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createOpenNexusCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createOpenNexusCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createOpenNexusCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createOpenNexusCodingTools({
      config: cfg,
    ...
    const bobTools = createOpenNexusCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createOpenNexusCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createOpenNexusCodingTools({
      config: cfg,
