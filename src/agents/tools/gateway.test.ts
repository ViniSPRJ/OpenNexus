...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    opennexus: process.env.OPENNEXUS_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.opennexus === undefined) {
      delete process.env.OPENNEXUS_GATEWAY_TOKEN;
    } else {
      process.env.OPENNEXUS_GATEWAY_TOKEN = envSnapshot.opennexus;
    }
  ...
  it("uses OPENNEXUS_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.OPENNEXUS_GATEWAY_TOKEN = "local-env-token";
