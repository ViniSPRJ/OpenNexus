...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OpenNexusConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OpenNexusConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OpenNexusConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OpenNexusConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: OpenNexusConfig, accountId = "media") {
    await handleTelegramAction(
