...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OPENNEXUS_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".opennexus", `workspace-${profile}`);
  }
  return path.join(home, ".opennexus", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".opennexus";
