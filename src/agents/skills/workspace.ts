...
function resolveSkillsLimits(config?: OpenNexusConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "opennexus-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "opennexus-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveOpenNexusMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: OpenNexusConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`opennexus skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
