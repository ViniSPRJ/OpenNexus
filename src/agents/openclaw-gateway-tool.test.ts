...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createOpenNexusTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-test-"));

    try {
      await withEnvAsync(
        { OPENNEXUS_STATE_DIR: stateDir, OPENNEXUS_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: opennexus --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/opennexus" } }\n}\n';
