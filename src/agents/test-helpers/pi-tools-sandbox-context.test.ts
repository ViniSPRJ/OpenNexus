...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("opennexus-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("opennexus-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("opennexus-sbx-");
