...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPENNEXUS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPENNEXUS_HOME: "/srv/opennexus-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/opennexus-home"), ".opennexus", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".opennexus", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("opennexus-workspace-");
