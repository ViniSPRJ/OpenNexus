...
type CoreToolDefinition = {
  ...
  includeInOpenNexusGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const opennexusTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInOpenNexusGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:opennexus": opennexusTools,
