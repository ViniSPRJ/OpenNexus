...
const GOOGLE_LIVE_SINGLE_KEY = "OPENNEXUS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OPENNEXUS_LIVE_ANTHROPIC_KEY",
    listVar: "OPENNEXUS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OPENNEXUS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OPENNEXUS_LIVE_${base}_KEY`;
