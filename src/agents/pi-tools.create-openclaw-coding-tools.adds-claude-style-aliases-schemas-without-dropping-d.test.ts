...
describe("createOpenNexusCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-read-"));
    try {
      ...
      const contents = "Hello from opennexus read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createOpenNexusCodingTools({ sandbox });
