...
function resolveExecConfig(params: { cfg?: OpenNexusConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: OpenNexusConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createOpenNexusCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createOpenNexusReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
