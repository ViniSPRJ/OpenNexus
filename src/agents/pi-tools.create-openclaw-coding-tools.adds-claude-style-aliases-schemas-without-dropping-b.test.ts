...
const defaultTools = createOpenNexusCodingTools({ senderIsOwner: true });
...
describe("createOpenNexusCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: OpenNexusConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createOpenNexusCodingTools({
    ...
    const anthropicTools = createOpenNexusCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: OpenNexusConfig = {
      tools: {
        exec: {
    ...
    const allowed = createOpenNexusCodingTools({
    ...
    const denied = createOpenNexusCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createOpenNexusCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
