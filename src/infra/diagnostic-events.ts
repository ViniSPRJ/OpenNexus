...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __opennexusDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__opennexusDiagnosticEventsState) {
    globalStore.__opennexusDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__opennexusDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: OpenNexusConfig): boolean {
