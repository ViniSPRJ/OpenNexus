...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENNEXUS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*opennexus-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "opennexus";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "opennexus");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "opennexus-gw");
  ...
  records.push(`_opennexus-gw._tcp IN PTR ${instanceLabel}._opennexus-gw._tcp`);
  records.push(`${instanceLabel}._opennexus-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._opennexus-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; opennexus-content-hash: ${contentHash}\n${contentBody}`;
}
