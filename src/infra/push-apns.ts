...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.OPENNEXUS_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.OPENNEXUS_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set OPENNEXUS_APNS_TEAM_ID and OPENNEXUS_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.OPENNEXUS_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.OPENNEXUS_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.OPENNEXUS_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set OPENNEXUS_APNS_PRIVATE_KEY_P8 or OPENNEXUS_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading OPENNEXUS_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createOpenNexusPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    opennexus: createOpenNexusPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
