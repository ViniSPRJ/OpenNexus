...
import { resolveOpenNexusPackageRoot, resolveOpenNexusPackageRootSync } from "./opennexus-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveOpenNexusPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "opennexus" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveOpenNexusPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // opennexus.mjs or dist/<bundle>.js
