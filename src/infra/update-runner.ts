...
const DEFAULT_PACKAGE_NAME = "opennexus";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "opennexus.mjs");
    ...
    const doctorStep = await runStep(
      step("opennexus doctor", doctorArgv, gitRoot, { OPENNEXUS_UPDATE_IN_PROGRESS: "1" }),
    );
