...
function candidateBinDirs(opts: EnsureOpenNexusPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `opennexus` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "opennexus");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OPENNEXUS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "opennexus"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `opennexus` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenNexusCliOnPath(opts: EnsureOpenNexusPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENNEXUS_PATH_BOOTSTRAPPED)) {
    return;
  }
