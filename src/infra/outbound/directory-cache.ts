...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OpenNexusConfig | null = null;
  ...
  get(key: string, cfg: OpenNexusConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OpenNexusConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: OpenNexusConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: OpenNexusConfig): void {
