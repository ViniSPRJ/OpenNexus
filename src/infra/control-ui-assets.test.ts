...
vi.mock("./opennexus-root.js", () => ({
  resolveOpenNexusPackageRoot: vi.fn(async () => null),
  resolveOpenNexusPackageRootSync: vi.fn(() => null),
}));
...
let opennexusRoot: typeof import("./opennexus-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    opennexusRoot = await import("./opennexus-root.js");
  });
  ...
  it("uses resolveOpenNexusPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/opennexus");
    (
      opennexusRoot.resolveOpenNexusPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/opennexus"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "opennexus" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "opennexus.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-opennexus");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/opennexus-bundle");
    (
      opennexusRoot.resolveOpenNexusPackageRootSync as unknown as ReturnType<typeof vi.fn>
