...
import { POSIX_OPENNEXUS_TMP_DIR, resolvePreferredOpenNexusTmpDir } from "./tmp-opennexus-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOpenNexusTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `opennexus-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOpenNexusTmpDir({
    accessSync,
...
describe("resolvePreferredOpenNexusTmpDir", () => {
  it("prefers /tmp/opennexus when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_OPENNEXUS_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/opennexus when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OPENNEXUS_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/opennexus when /tmp/opennexus is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/opennexus when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/opennexus is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/opennexus is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/opennexus is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
