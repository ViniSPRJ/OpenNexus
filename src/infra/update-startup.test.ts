...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["OPENNEXUS_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.OPENNEXUS_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOpenNexusPackageRoot } = await import("./opennexus-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveOpenNexusPackageRoot).mockResolvedValue("/opt/opennexus");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/opennexus/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
