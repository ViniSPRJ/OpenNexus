...
export function triggerOpenNexusRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENNEXUS_SYSTEMD_UNIT,
        process.env.OPENNEXUS_PROFILE,
      );
  ...
  const label =
    process.env.OPENNEXUS_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENNEXUS_PROFILE);
