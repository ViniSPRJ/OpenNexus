...
let ensureOpenNexusCliOnPath: typeof import("./path-env.js").ensureOpenNexusCliOnPath;

describe("ensureOpenNexusCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OPENNEXUS_PATH_BOOTSTRAPPED",
    "OPENNEXUS_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOpenNexusCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling opennexus exists", () => {
    const tmp = abs("/tmp/opennexus-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "opennexus");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.OPENNEXUS_PATH_BOOTSTRAPPED;

    ensureOpenNexusCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.OPENNEXUS_PATH_BOOTSTRAPPED = "1";
    ensureOpenNexusCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/opennexus-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "opennexus");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/opennexus-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "opennexus");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "opennexus");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/opennexus-path/case-linuxbrew");
