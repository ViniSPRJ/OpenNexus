...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ OPENNEXUS_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-opennexus",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-opennexus#general");
  });
  ...
  it("derives session transcripts dir from OPENNEXUS_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPENNEXUS_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.opennexus");
  ...
  it("resolves cross-agent paths when OPENNEXUS_STATE_DIR differs from stored paths", () => {
  ...
  it("falls back to derived transcript path when sessionFile is outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.opennexus"), () => {
      const sessionFile = resolveSessionFilePath(
        "sess-1",
      ...
      expect(sessionFile).toBe(
        path.join(
          path.resolve("/home/user/.opennexus"),
          "agents",
          "bot1",
          "sessions",
          "sess-1.jsonl",
