...
import type { OpenNexusConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpenNexusConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OpenNexusConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenNexusSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenNexusConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenNexusConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as OpenNexusConfig,
  };
}
