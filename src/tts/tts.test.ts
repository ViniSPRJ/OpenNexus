...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.OPENNEXUS_TTS_PREFS;
      process.env.OPENNEXUS_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      try {
        ...
        process.env.OPENNEXUS_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: OpenNexusConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
