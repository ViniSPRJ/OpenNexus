...
const SEARCH_TOOL = "https://docs.opennexus.ai/mcp.SearchOpenNexus";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.opennexus.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('opennexus docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.opennexus.ai/");
      runtime.log(`Search: ${formatCliCommand('opennexus docs "your query"')}`);
    }
    return;
  }
