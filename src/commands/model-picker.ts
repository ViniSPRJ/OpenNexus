...
type PromptDefaultModelResult = { model?: string; config?: OpenNexusConfig };
...
function resolveConfiguredModelRaw(cfg: OpenNexusConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: OpenNexusConfig): string[] {
...
export function applyPrimaryModel(cfg: OpenNexusConfig, model: string): OpenNexusConfig {
...
export function applyModelAllowlist(cfg: OpenNexusConfig, models: string[]): OpenNexusConfig {
  const defaults = cfg.agents?.defaults;
