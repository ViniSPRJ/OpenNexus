...
import type { OpenNexusConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenNexusConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("opennexus security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.opennexus/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("opennexus approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.opennexus.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("opennexus configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("opennexus config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("opennexus doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("opennexus config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('opennexus config set session.dmScope "per-channel-peer"') +
