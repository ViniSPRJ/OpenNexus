...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenNexus status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("opennexus security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("opennexus security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.opennexus.ai/faq");
  runtime.log("Troubleshooting: https://docs.opennexus.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("opennexus status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("opennexus logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("opennexus status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("opennexus gateway probe")}`);
  }
}
