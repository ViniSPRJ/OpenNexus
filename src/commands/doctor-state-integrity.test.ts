...
type EnvSnapshot = {
  HOME?: string;
  OPENNEXUS_HOME?: string;
  OPENNEXUS_STATE_DIR?: string;
  OPENNEXUS_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    OPENNEXUS_HOME: process.env.OPENNEXUS_HOME,
    OPENNEXUS_STATE_DIR: process.env.OPENNEXUS_STATE_DIR,
    OPENNEXUS_OAUTH_DIR: process.env.OPENNEXUS_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: OpenNexusConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: OpenNexusConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "opennexus-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.OPENNEXUS_HOME = tempHome;
    process.env.OPENNEXUS_STATE_DIR = path.join(tempHome, ".opennexus");
    delete process.env.OPENNEXUS_OAUTH_DIR;
    fs.mkdirSync(process.env.OPENNEXUS_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when OPENNEXUS_OAUTH_DIR is explicitly configured", async () => {
    process.env.OPENNEXUS_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OpenNexusConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints opennexus-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: OpenNexusConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/opennexus sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/opennexus sessions cleanup --store ".*sessions\.json" --dry-run/);
