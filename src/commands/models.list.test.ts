...
const ensureOpenNexusModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/opennexus-agent/auth.json" });
const resolveOpenNexusAgentDir = vi.fn().mockReturnValue("/tmp/opennexus-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/opennexus-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureOpenNexusModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/opennexus-agent");
  });
