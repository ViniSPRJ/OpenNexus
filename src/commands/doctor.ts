...
function resolveMode(cfg: OpenNexusConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenNexus doctor");

  const root = await resolveOpenNexusPackageRoot({
  ...
  let cfg: OpenNexusConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("opennexus configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("opennexus config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("opennexus setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("opennexus doctor --fix")}" to apply changes.`);
  }
