...
export function requireOpenNexusAgentDir(): string {
  const agentDir = process.env.OPENNEXUS_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENNEXUS_AGENT_DIR not set");
  }
  return agentDir;
}
