...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENNEXUS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENNEXUS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENNEXUS_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENNEXUS_GATEWAY_PASSWORD");
  });
