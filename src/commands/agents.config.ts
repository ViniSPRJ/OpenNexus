...
function resolveAgentName(cfg: OpenNexusConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenNexusConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenNexusConfig): AgentSummary[] {
