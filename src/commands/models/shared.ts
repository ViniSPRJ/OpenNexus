...
export async function loadValidConfigOrThrow(): Promise<OpenNexusConfig> {
...
export async function updateConfig(
  mutator: (cfg: OpenNexusConfig) => OpenNexusConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenNexusConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenNexusConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("opennexus agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
