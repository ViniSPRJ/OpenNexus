...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenNexusAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("opennexus models auth setup-token")}\` or \`${formatCliCommand("opennexus configure")}\`.`
          : `Run \`${formatCliCommand("opennexus configure")}\` or set an API key env var.`;
