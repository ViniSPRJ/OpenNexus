...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenNexusAgentDir: vi.fn().mockReturnValue("/tmp/opennexus-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/opennexus-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/opennexus-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenNexusAgentDir: mocks.resolveOpenNexusAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/opennexus-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenNexusAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/opennexus-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/opennexus-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/opennexus-agent-custom");
