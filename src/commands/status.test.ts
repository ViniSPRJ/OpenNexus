...
beforeAll(() => {
  envSnapshot = captureEnv(["OPENNEXUS_PROFILE"]);
  process.env.OPENNEXUS_PROFILE = "isolated";
});
...
vi.mock("../infra/opennexus-root.js", () => ({
  resolveOpenNexusPackageRoot: vi.fn().mockResolvedValue("/tmp/opennexus"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/opennexus/pnpm-lock.yaml",
      markerPath: "/tmp/opennexus/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("OpenNexus status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("opennexus status --all") ||
          l.includes("opennexus --profile isolated status --all") ||
          l.includes("opennexus status --all") ||
          l.includes("opennexus --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OPENNEXUS_GATEWAY_TOKEN = "abcd1234";
    try {
