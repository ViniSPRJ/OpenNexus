...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: OpenNexusConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: OpenNexusConfig) => { changed: boolean; next: OpenNexusConfig };
  defaultModel: string;
  overrideConfig: OpenNexusConfig;
  alreadyDefaultConfig: OpenNexusConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: OpenNexusConfig) => config,
      applyDefaultConfig: (config: OpenNexusConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: OpenNexusConfig = { agents: { defaults: {} } };
