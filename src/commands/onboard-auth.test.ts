...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OPENNEXUS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("opennexus-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-oauth-sync-"));
    ...
    process.env.OPENNEXUS_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-oauth-nosync-"));
    process.env.OPENNEXUS_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OPENNEXUS_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opennexus-oauth-external-"));
    process.env.OPENNEXUS_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OPENNEXUS_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENNEXUS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("opennexus-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
