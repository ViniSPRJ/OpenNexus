 ...
 * Runs as part of `opennexus doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("opennexus config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("opennexus memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("opennexus configure")}`,
        `- To disable: ${formatCliCommand("opennexus config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("opennexus memory status --deep")}`,
      ].join("\n"),
      "Memory search",
