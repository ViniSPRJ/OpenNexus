...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("opennexus-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("opennexus-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("opennexus-lid-a-", (first) => {
      withTempDirSync("opennexus-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.opennexus when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "opennexus-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENNEXUS_HOME over HOME", () => {
    vi.stubEnv("OPENNEXUS_HOME", "/srv/opennexus-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/opennexus-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENNEXUS_HOME prefix when OPENNEXUS_HOME is set", () => {
    vi.stubEnv("OPENNEXUS_HOME", "/srv/opennexus-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/opennexus-home")}/.opennexus/opennexus.json`)).toBe(
      "$OPENNEXUS_HOME/.opennexus/opennexus.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENNEXUS_HOME replacement when OPENNEXUS_HOME is set", () => {
    vi.stubEnv("OPENNEXUS_HOME", "/srv/opennexus-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/opennexus-home")}/.opennexus/opennexus.json`),
    ).toBe("config: $OPENNEXUS_HOME/.opennexus/opennexus.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/opennexus")).toBe(path.resolve(os.homedir(), "opennexus"));
  });
  ...
  it("prefers OPENNEXUS_HOME for tilde expansion", () => {
    vi.stubEnv("OPENNEXUS_HOME", "/srv/opennexus-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/opennexus")).toBe(path.resolve("/srv/opennexus-home", "opennexus"));
