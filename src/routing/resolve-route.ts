...
function listAgents(cfg: OpenNexusConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: OpenNexusConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OpenNexusConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<OpenNexusConfig, EvaluatedBindingsCache>();
