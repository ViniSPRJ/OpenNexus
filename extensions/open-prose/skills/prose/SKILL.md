...
metadata: { "opennexus": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OpenNexus `sessions_spawn`
- **File I/O** == OpenNexus `read`/`write`
- **Remote fetch** == OpenNexus `web_fetch` (or `exec` with curl when POST is required)
