...
`opennexus.invoke --each`) without adding new OpenNexus code per workflow.
...
This extension depends on OpenNexus internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenNexus extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.opennexus/extensions` as a standalone plugin directory.
