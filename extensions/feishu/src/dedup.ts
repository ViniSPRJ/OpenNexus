...
import { createDedupeCache, createPersistentDedupe } from "opennexus/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.OPENNEXUS_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["opennexus-vitest", String(process.pid)].join("-"));
  }
