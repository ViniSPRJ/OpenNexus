...
function resolveOpenNexusRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENNEXUS_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPENNEXUS_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpenNexusRoot(), "dist", "extensionAPI.js");
