...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["opennexusPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["opennexus", "pollId"]) ??
    readNestedString(value, ["opennexus", "poll", "id"]) ??
    readNestedString(value, ["data", "opennexusPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "opennexus", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          opennexusPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "opennexus poll vote",
          displayText: "Vote recorded",
          value: { opennexusPollId: pollId, pollId },
        },
      },
    ],
  };
