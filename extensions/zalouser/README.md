...
OpenNexus extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
opennexus channels login --channel zalouser
...
```bash
opennexus message send --channel zalouser --target <threadId> --message "Hello from OpenNexus!"
```
...
```bash
opennexus channels login --channel zalouser              # Login via QR
opennexus channels login --channel zalouser --account work
opennexus channels status --probe
opennexus channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
opennexus directory self --channel zalouser
opennexus directory peers list --channel zalouser --query "name"
opennexus directory groups list --channel zalouser --query "work"
opennexus directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
opennexus message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
opennexus message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `opennexus channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
opennexus channels login --channel zalouser --account work
opennexus message send --channel zalouser --account work --target <id> --message "Hello"
