...
import type { ReplyPayload, OpenNexusConfig } from "opennexus/plugin-sdk";
import { createReplyPrefixOptions } from "opennexus/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // OpenNexusConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as OpenNexusConfig;
