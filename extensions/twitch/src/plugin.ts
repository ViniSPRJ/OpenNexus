/**
 * Twitch channel plugin for OpenNexus.
 *
...
import { buildChannelConfigSchema } from "opennexus/plugin-sdk";
 ...
 * for OpenNexus. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OpenNexusConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OpenNexusConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OpenNexusConfig): boolean => {
