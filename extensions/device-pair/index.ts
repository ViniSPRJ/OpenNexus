...
function resolveGatewayPort(cfg: OpenNexusPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.OPENNEXUS_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: OpenNexusPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: OpenNexusPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the OpenNexus iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the OpenNexus iOS app:",
            "",
            "```",
            qrAscii,
            "```",
