...
import type { OpenNexusPluginApi, OpenNexusPluginService } from "opennexus/plugin-sdk";
...
function normalizeDenyList(cfg: OpenNexusPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpenNexusPluginApi["config"]): string[] {
...
export default function register(api: OpenNexusPluginApi) {
  ...
  const timerService: OpenNexusPluginService = {
