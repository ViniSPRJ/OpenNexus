...
If you believe you've found a security issue in OpenNexus, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [opennexus/opennexus](https://github.com/opennexus/opennexus)
- **macOS desktop app** — [opennexus/opennexus](https://github.com/opennexus/opennexus) (apps/macos)
- **iOS app** — [opennexus/opennexus](https://github.com/opennexus/opennexus) (apps/ios)
- **Android app** — [opennexus/opennexus](https://github.com/opennexus/opennexus) (apps/android)
- **ClawHub** — [opennexus/clawhub](https://github.com/opennexus/clawhub)
- **Trust and threat model** — [opennexus/trust](https://github.com/opennexus/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@opennexus.ai](mailto:security@opennexus.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.opennexus.ai).
...
- Tested version details (OpenNexus version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to OpenNexus's documented trust boundaries.
- For exposed-secret reports: proof the credential is OpenNexus-owned (or grants access to OpenNexus-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at OpenNexus. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

OpenNexus is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
OpenNexus does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- OpenNexus can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need OpenNexus, use one VPS (or host/OS user boundary) per user.
...
- Using OpenNexus in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.opennexus`, workspace files like `MEMORY.md` / `memory/*.md`)
- Any report whose only claim is that an operator-enabled `dangerous*`/`dangerously*` config option weakens defaults (these are explicit break-glass tradeoffs by design)
- Reports that depend on trusted operator-supplied configuration values to trigger availability impact (for example custom regex patterns). These may still be fixed as defense-in-depth hardening, but are not security-boundary bypasses.
- Exposed secrets that are third-party/user-controlled credentials (not OpenNexus-owned and not granting access to OpenNexus-operated infrastructure/services) without demonstrated OpenNexus impact

## Deployment Assumptions

OpenNexus security guidance assumes:

- The host where OpenNexus runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.opennexus` state/config (including `opennexus.json`) is effectively a trusted operator.
...
OpenNexus's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
- Plugins can execute with the same OS privileges as the OpenNexus process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Operational Guidance

For threat model + hardening guidance (including `opennexus security audit --deep` and `--fix`), see:

- `https://docs.opennexus.ai/gateway/security`
...
OpenNexus's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `opennexus gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - OpenNexus keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `opennexus security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__opennexus__/canvas/`, `/__opennexus__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__opennexus__/canvas/`, `/__opennexus__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

OpenNexus requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running OpenNexus in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v opennexus-data:/app/data \
  opennexus/opennexus:latest
```
