...
struct ChatMessageBubble: View {
    let message: OpenNexusChatMessage
    let style: OpenNexusChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? OpenNexusChatTheme.userText : OpenNexusChatTheme.assistantText
    ...
    private var inlineAttachments: [OpenNexusChatMessageContent] {
    ...
    private var toolCalls: [OpenNexusChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [OpenNexusChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? OpenNexusChatTheme.userBubble
        }
        if self.style == .onboarding {
            return OpenNexusChatTheme.onboardingAssistantBubble
        }
        return OpenNexusChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return OpenNexusChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: OpenNexusChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? OpenNexusChatTheme.userText : OpenNexusChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: OpenNexusChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OpenNexusChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(OpenNexusChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [OpenNexusChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: OpenNexusChatTheme.assistantText)
            }
        }
    }
}
