...
@MainActor
struct OpenNexusChatComposer: View {
    @Bindable var viewModel: OpenNexusChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(OpenNexusChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(OpenNexusChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \OpenNexusPendingAttachment.id)
                { (att: OpenNexusPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            OpenNexusPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(OpenNexusChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OpenNexusChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(OpenNexusChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(OpenNexusChatTheme.subtleCard)
        .clipShape(Capsule())
    }
