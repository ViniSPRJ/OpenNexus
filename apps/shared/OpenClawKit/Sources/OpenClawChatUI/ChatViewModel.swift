...
private let chatUILogger = Logger(subsystem: "ai.opennexus", category: "OpenNexusChatUI")

@MainActor
@Observable
public final class OpenNexusChatViewModel {
    public private(set) var messages: [OpenNexusChatMessage] = []
    ...
    public var attachments: [OpenNexusPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OpenNexusChatPendingToolCall] = []
    public private(set) var sessions: [OpenNexusChatSessionEntry] = []
    private let transport: any OpenNexusChatTransport
    ...
    private var pendingToolCallsById: [String: OpenNexusChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OpenNexusChatTransport) {
    ...
    public var sessionChoices: [OpenNexusChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [OpenNexusChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OpenNexusPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OpenNexusChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OpenNexusChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: OpenNexusChatMessage) -> OpenNexusChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> OpenNexusChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return OpenNexusChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: OpenNexusChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [OpenNexusChatMessage],
        incoming: [OpenNexusChatMessage]) -> [OpenNexusChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [OpenNexusChatMessage]) -> [OpenNexusChatMessage] {
        var result: [OpenNexusChatMessage] = []
    ...
    private static func dedupeKey(for message: OpenNexusChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OpenNexusChatMessageContent] = [
            OpenNexusChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> OpenNexusChatAttachmentPayload in
            OpenNexusChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OpenNexusChatSessionEntry {
        OpenNexusChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OpenNexusChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OpenNexusChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OpenNexusAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OpenNexusChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OpenNexusPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
