...
enum CommandResolver {
    private static let projectRootDefaultsKey = "opennexus.gatewayProjectRootPath"
    private static let helperName = "opennexus"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let opennexusEntry = root.appendingPathComponent("opennexus.mjs").path
        if FileManager().isReadableFile(atPath: opennexusEntry) { return opennexusEntry }
        let binEntry = root.appendingPathComponent("bin/opennexus.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/opennexus")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let opennexusPaths = self.opennexusManagedPaths(home: home)
        if !opennexusPaths.isEmpty {
            extras.insert(contentsOf: opennexusPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + opennexusPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func opennexusManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".opennexus"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func opennexusExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOpenNexusExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("opennexus.mjs").path,
            root.appendingPathComponent("bin/opennexus.js").path,
        ]
    ...
    static func hasAnyOpenNexusInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.opennexusExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func opennexusNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let opennexusPath = self.projectOpenNexusExecutable(projectRoot: root) {
                return [opennexusPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "opennexus", subcommand] + extraArgs
            }
            if let opennexusPath = self.opennexusExecutable(searchPaths: searchPaths) {
                return [opennexusPath, subcommand] + extraArgs
            }

            let missingEntry = """
            opennexus entrypoint missing (looked for dist/index.js or opennexus.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real opennexus CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/opennexus"
        ...
        \(cliSection)
        \(projectSection)
        if command -v opennexus >/dev/null 2>&1; then
          CLI="$(command -v opennexus)"
          opennexus \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/opennexus.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/opennexus.mjs"
            node "$PRJ/opennexus.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/opennexus.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/opennexus.js"
            node "$PRJ/bin/opennexus.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent opennexus"
          pnpm --silent opennexus \(quotedArgs);
        else
          echo "opennexus CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OpenNexusConfigFile.loadDict()
