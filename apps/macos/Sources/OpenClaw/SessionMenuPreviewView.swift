...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.opennexus", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenNexusSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenNexusSessionPreviewEntry,
    ...
    private static func cache(payload: OpenNexusSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenNexusChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpenNexusKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpenNexusKit.AnyCodable]) -> [OpenNexusChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenNexusChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenNexusChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenNexusChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenNexusChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenNexusChatMessage) -> String? {
