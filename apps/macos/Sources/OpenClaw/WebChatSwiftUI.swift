...
private let webChatSwiftLogger = Logger(subsystem: "ai.opennexus", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: OpenNexusChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OpenNexusChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(OpenNexusChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OpenNexusChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OpenNexusGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    OpenNexusChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    OpenNexusAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OpenNexusChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OpenNexusChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OpenNexusChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: OpenNexusChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OpenNexusChatView>) -> NSViewController
    {
