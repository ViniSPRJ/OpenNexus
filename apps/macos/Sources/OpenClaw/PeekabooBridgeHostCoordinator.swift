...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.opennexus", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OpenNexusPeekabooBridgeServices?
    private static var opennexusSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("OpenNexus", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = OpenNexusPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.opennexusSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.opennexusSocketPath, privacy: .public)")
    }
...
@MainActor
private final class OpenNexusPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.opennexus.peekaboo")
