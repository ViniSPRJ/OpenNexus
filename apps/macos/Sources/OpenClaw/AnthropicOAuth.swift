...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (OpenNexus token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: OpenNexusOAuthStore.AnthropicOAuthStatus = OpenNexusOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.opennexus", category: "anthropic-oauth")
...
enum OpenNexusOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let opennexusOAuthDirEnv = "OPENNEXUS_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "OpenNexus OAuth token file not found"
            case .unreadableFile: "OpenNexus OAuth token file not readable"
            case .invalidJSON: "OpenNexus OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in OpenNexus OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "OpenNexus OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.opennexusOAuthDirEnv]?
