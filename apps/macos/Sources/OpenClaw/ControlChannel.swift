...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpenNexusProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.opennexus", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OpenNexusKit.AnyCodable]()) {
                $0[$1.key] = OpenNexusKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpenNexusProtocol.AnyCodable?) -> [String: OpenNexusProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpenNexusProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpenNexusKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpenNexusProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("opennexus.control.heartbeat")
    static let controlAgentEvent = Notification.Name("opennexus.control.agent")
}
