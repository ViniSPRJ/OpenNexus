...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.opennexus", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous OpenNexus run already has an SSH listener on the expected port (common after restarts),
