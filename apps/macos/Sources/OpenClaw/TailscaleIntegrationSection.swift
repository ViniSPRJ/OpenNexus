...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.opennexus/opennexus.json. Prefer OPENNEXUS_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.opennexus/opennexus.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.opennexus/opennexus.json. Restart the gateway to apply."
        }
