...
enum OpenNexusLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OpenNexusOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpenNexusFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpenNexusLogging.bootstrapIfNeeded()
        let label = OpenNexusLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpenNexusOSLogHandler: LogHandler {
...
struct OpenNexusFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpenNexusLogging.parseLabel(self.label)
