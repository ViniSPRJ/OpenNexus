...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.opennexus", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: OpenNexusNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [OpenNexusCapability.canvas.rawValue, OpenNexusCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(OpenNexusCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if OpenNexusLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(OpenNexusCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            OpenNexusSystemCommand.which.rawValue,
            OpenNexusSystemCommand.run.rawValue,
            OpenNexusSystemCommand.execApprovalsGet.rawValue,
            OpenNexusSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(OpenNexusCapability.camera.rawValue) {
        ...
        if capsSet.contains(OpenNexusCapability.location.rawValue) {
