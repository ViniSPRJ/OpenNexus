...
enum DebugActions {
    private static let verboseDefaultsKey = "opennexus.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "opennexus.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OpenNexusPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OpenNexus", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OpenNexusPaths.configURL
        guard
