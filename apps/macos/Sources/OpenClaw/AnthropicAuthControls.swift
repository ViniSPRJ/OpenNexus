...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: OpenNexusOAuthStore.AnthropicOAuthStatus = OpenNexusOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: OpenNexusOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = OpenNexusOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = OpenNexusOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. OpenNexus can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: OpenNexusOAuthStore.AnthropicOAuthStatus,
