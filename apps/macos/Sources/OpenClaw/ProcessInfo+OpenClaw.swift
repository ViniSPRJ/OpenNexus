...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.opennexus.mac`) even if the shipped
    ...
    {
        if environment["OPENNEXUS_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "opennexus.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "opennexus.nixMode") { return true }
