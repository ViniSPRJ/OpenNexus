...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPENNEXUS_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/opennexus")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("opennexus-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("opennexus-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/opennexus/ (rolling or stdout), or nil if none exist.
