...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: OpenNexusProtocol.AnyCodable(["ok": OpenNexusProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: OpenNexusProtocol.AnyCodable(["ok": OpenNexusProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = OpenNexusProtocol.AnyCodable([
            "runId": OpenNexusProtocol.AnyCodable("run-1"),
            "sessionKey": OpenNexusProtocol.AnyCodable("main"),
            "state": OpenNexusProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: OpenNexusProtocol.AnyCodable(["a": OpenNexusProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
