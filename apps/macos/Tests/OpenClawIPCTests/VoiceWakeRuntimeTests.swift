...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["opennexus"]
        let text = "hey opennexus"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey opennexus do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["opennexus"], minPostTriggerGap: 0.3)
