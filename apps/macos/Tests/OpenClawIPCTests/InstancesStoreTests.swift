...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OpenNexusProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OpenNexusProtocol.AnyCodable] = [
            "presence": .init([OpenNexusProtocol.AnyCodable(entry)]),
        ]
        let payload = OpenNexusProtocol.AnyCodable(payloadMap)
