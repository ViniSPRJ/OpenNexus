...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.opennexus.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to OpenNexus", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to OpenNexus gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to OpenNexus.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "OpenNexusShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "OpenNexus is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "OpenNexus Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("opennexus-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
