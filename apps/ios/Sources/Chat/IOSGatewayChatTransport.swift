...
struct IOSGatewayChatTransport: OpenNexusChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.opennexus", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> OpenNexusChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OpenNexusChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OpenNexusChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OpenNexusChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OpenNexusChatAttachmentPayload]) async throws -> OpenNexusChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OpenNexusChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(OpenNexusChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(OpenNexusGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenNexusGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenNexusChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenNexusAgentEventPayload.self)
                        {
