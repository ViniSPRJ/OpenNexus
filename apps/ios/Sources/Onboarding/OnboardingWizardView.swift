...
struct OnboardingWizardView: View {
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Connect to your OpenNexus gateway")
                .font(.subheadline)
                .foregroundStyle(.secondary)
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `opennexus devices list`."
                    }()
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `opennexus devices approve` (or `opennexus devices approve <requestId>`)\n"
                            + "2) `/pair approve` in Telegram\n"
                            + "\(requestLine)\n"
                            + "OpenNexus will also retry automatically when you return to this app.")
                }
            }
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if let last = GatewaySettingsStore.loadLastGatewayConnection() {
                ...
                case .discovered:
                    self.manualHost = "opennexus.local"
        ...
        if self.selectedMode == .developerLocal && self.manualHost == "opennexus.local" {
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "opennexus.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "opennexus.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "opennexus.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
