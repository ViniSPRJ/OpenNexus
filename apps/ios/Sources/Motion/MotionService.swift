...
final class MotionService: MotionServicing {
    func activities(params: OpenNexusMotionActivityParams) async throws -> OpenNexusMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OpenNexusMotionActivityEntry], Error>) in
        ...
        return OpenNexusMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OpenNexusPedometerParams) async throws -> OpenNexusPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<OpenNexusPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OpenNexusPedometerPayload(
