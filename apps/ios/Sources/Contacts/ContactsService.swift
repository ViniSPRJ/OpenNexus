...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OpenNexusContactsSearchParams) async throws -> OpenNexusContactsSearchPayload {
        ...
        return OpenNexusContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OpenNexusContactsAddParams) async throws -> OpenNexusContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OpenNexusContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OpenNexusContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OpenNexusContactPayload {
        OpenNexusContactPayload(
