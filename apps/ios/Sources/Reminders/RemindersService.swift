...
final class RemindersService: RemindersServicing {
    func list(params: OpenNexusRemindersListParams) async throws -> OpenNexusRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OpenNexusReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return OpenNexusReminderPayload(
        ...
        return OpenNexusRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OpenNexusRemindersAddParams) async throws -> OpenNexusRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OpenNexusReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OpenNexusRemindersAddPayload(reminder: payload)
    }
