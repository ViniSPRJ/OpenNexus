...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> OpenNexusNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> OpenNexusNetworkStatusPayload {
        let status: OpenNexusNetworkPathStatus = switch path.status {
        ...
        var interfaces: [OpenNexusNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return OpenNexusNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> OpenNexusNetworkStatusPayload {
        OpenNexusNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
