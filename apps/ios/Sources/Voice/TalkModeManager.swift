...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__OPENNEXUS_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<OpenNexusTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = OpenNexusTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> OpenNexusTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return OpenNexusTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> OpenNexusTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> OpenNexusTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return OpenNexusTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> OpenNexusTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = OpenNexusTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: OpenNexusTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: OpenNexusAgentEventPayload.self) else {
                continue
            }
