---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenNexus deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenNexus cron scheduling for periodic checks, or version status checks on a machine running OpenNexus (laptop, workstation, Pi, VPS).
---

# OpenNexus Host Hardening

## Overview

Assess and harden the host running OpenNexus, then align it to a user-defined risk tolerance without breaking access. Use OpenNexus security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenNexus changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenNexus gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenNexus can access sensitive data.
...
### 2) Run OpenNexus security audits (read-only)

As part of the default read-only checks, run `opennexus security audit --deep`. Only offer alternatives if the user requests them:

1. `opennexus security audit` (faster, non-probing)
2. `opennexus security audit --json` (structured output)

Offer to apply OpenNexus safe defaults (numbered):

1. `opennexus security audit --fix`

Be explicit that `--fix` only tightens OpenNexus defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenNexus version/update status (read-only)

As part of the default read-only checks, run `opennexus update status`.
...
- Credential hygiene notes (location of OpenNexus creds, prefer disk encryption)
...
- OpenNexus security audit (re-run)
...
After OpenNexus install or first hardening pass, run at least one baseline audit and version check:

- `opennexus security audit`
- `opennexus security audit --deep`
- `opennexus update status`

Ongoing monitoring is recommended. Use the OpenNexus cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `opennexus cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `opennexus update status`
...
Before creating, `opennexus cron list` and match on exact `name`. If found, `opennexus cron edit <id> ...`.
If not found, `opennexus cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `opennexus update status` (preferred for source checkouts and channels)
2. `npm view opennexus version` (published npm version)

## OpenNexus command accuracy

Use only supported commands and flags:

- `opennexus security audit [--deep] [--fix] [--json]`
- `opennexus status` / `opennexus status --deep`
- `opennexus health --json`
- `opennexus update status`
- `opennexus cron add|list|runs|run`

Do not invent CLI flags or imply OpenNexus enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenNexus compaction:
