...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `opennexus-e2e-${name}-`));
  const configDir = path.join(homeDir, ".opennexus");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENNEXUS_SKIP_PROVIDERS: "1",
          OPENNEXUS_SKIP_GMAIL_WATCHER: "1",
          OPENNEXUS_SKIP_CRON: "1",
          OPENNEXUS_SKIP_BROWSER_CONTROL_SERVER: "1",
          OPENNEXUS_SKIP_CANVAS_HOST: "1",
          OPENNEXUS_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
