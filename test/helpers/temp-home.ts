...
type EnvSnapshot = {
  ...
  opennexusHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    opennexusHome: process.env.OPENNEXUS_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OPENNEXUS_HOME", snapshot.opennexusHome);
  restoreKey("OPENNEXUS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OPENNEXUS_HOME.
  delete process.env.OPENNEXUS_HOME;
  process.env.OPENNEXUS_STATE_DIR = path.join(base, ".opennexus");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "opennexus-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".opennexus", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
