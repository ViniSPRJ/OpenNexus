...
export function connectGateway(host: GatewayHost) {
  ...
  const client = new GatewayBrowserClient({
    ...
    clientName: "opennexus-control-ui",
    mode: "webchat",
    ...
    onHello: (hello) => {
      ...
      void loadAssistantIdentity(host as unknown as OpenNexusApp);
      void loadAgents(host as unknown as OpenNexusApp);
      void loadToolsCatalog(host as unknown as OpenNexusApp);
      void loadNodes(host as unknown as OpenNexusApp, { quiet: true });
      void loadDevices(host as unknown as OpenNexusApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (state === "final") {
    void loadSessions(host as unknown as OpenNexusApp, {
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as OpenNexusApp, payload);
  handleTerminalChatEvent(host, payload, state);
  if (state === "final" && shouldReloadHistoryForFinalEvent(payload)) {
    void loadChatHistory(host as unknown as OpenNexusApp);
  }
}
