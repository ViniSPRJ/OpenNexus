...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/opennexus/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/opennexus");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/opennexus/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENNEXUS_CONTROL_UI_BASE_PATH__ = "/opennexus";
    const app = mountApp("/opennexus/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/opennexus");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/opennexus/sessions");
  });
