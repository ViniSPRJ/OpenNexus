...
export async function handleWhatsAppStart(host: OpenNexusApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OpenNexusApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OpenNexusApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OpenNexusApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OpenNexusApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OpenNexusApp): string {
...
function resolveGatewayHttpAuthHeader(host: OpenNexusApp): string | null {
...
function buildGatewayHttpHeaders(host: OpenNexusApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: OpenNexusApp) {
...
export function handleNostrProfileToggleAdvanced(host: OpenNexusApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OpenNexusApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OpenNexusApp) {
  const state = host.nostrProfileFormState;
