---
summary: "OpenNexus on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenNexus on a Raspberry Pi
  - Running OpenNexus on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenNexus on Raspberry Pi

## Goal

Run a persistent, always-on OpenNexus Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OpenNexus features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled opennexus

# Enable if not
sudo systemctl enable opennexus

# Start on boot
sudo systemctl start opennexus
```
...
```bash
# Check logs
journalctl -u opennexus --no-pager -n 100

# Common fix: rebuild
cd ~/opennexus  # if using hackable install
npm run build
sudo systemctl restart opennexus
```
