...
- Publish as `opennexus/plugin-sdk` (or export from core under `opennexus/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpenNexusPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenNexusConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenNexusConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenNexusConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenNexusConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenNexusConfig): string;
  };
};
```
...
- Introduce `opennexus/plugin-sdk`.
- Add `api.runtime` to `OpenNexusPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `opennexusRuntime: ">=2026.2.0"`).
