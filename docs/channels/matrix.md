...
Matrix is an open, decentralized messaging protocol. OpenNexus connects as a Matrix **user**
...
```bash
opennexus plugins install @opennexus/matrix
```

Local checkout (when running from a git repo):

```bash
opennexus plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `opennexus plugins install @opennexus/matrix`
   - From a local checkout: `opennexus plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenNexus calls the same
     login endpoint, stores the access token in `~/.opennexus/credentials/matrix/credentials.json`,
...
- On first connection, OpenNexus requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenNexus logs a warning.
...
`~/.opennexus/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `opennexus pairing list matrix`
  - `opennexus pairing approve matrix <CODE>`
...
- On startup, OpenNexus resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
