---
summary: "Mattermost bot setup and OpenNexus config"
read_when:
...
```bash
opennexus plugins install @opennexus/mattermost
```

Local checkout (when running from a git repo):

```bash
opennexus plugins install ./extensions/mattermost
```
...
4. Configure OpenNexus and start the gateway.
...
- Approve via:
  - `opennexus pairing list mattermost`
  - `opennexus pairing approve mattermost <CODE>`
...
Use these target formats with `opennexus message send` or cron/webhooks:
