...
```
    ...
    Telegram does **not** use `opennexus channels login telegram`; configure token in config/env, then start gateway.
...
opennexus pairing approve telegram <CODE>
```
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `opennexus doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `opennexus logs --follow`
...
- DM messages can carry `message_thread_id`; OpenNexus routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    OpenNexus can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, OpenNexus keeps the same preview message and performs a final edit in place (no second message).

    For complex replies (for example media payloads), OpenNexus falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, OpenNexus skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, OpenNexus retries as plain text.
...
```
    ...
    - `~/.opennexus/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, OpenNexus enqueues system events like:
...
```bash
opennexus message send --channel telegram --target 123456789 --message "hi"
opennexus message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `opennexus channels status` warns when config expects unmentioned group messages.
    - `opennexus channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `opennexus logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, OpenNexus now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `OPENNEXUS_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `OPENNEXUS_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `OPENNEXUS_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `opennexus doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `opennexus doctor --fix` can resolve legacy `@username` entries to IDs.
