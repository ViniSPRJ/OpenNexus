...
Tlon is a decentralized messenger built on Urbit. OpenNexus connects to your Urbit ship and can
...
```bash
opennexus plugins install @opennexus/tlon
```

Local checkout (when running from a git repo):

```bash
opennexus plugins install ./extensions/tlon
```
...
By default, OpenNexus blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `opennexus message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenNexus replies in-thread.
