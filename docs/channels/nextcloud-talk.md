...
```bash
opennexus plugins install @opennexus/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
opennexus plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenNexus" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `opennexus pairing list nextcloud-talk`
  - `opennexus pairing approve nextcloud-talk <CODE>`
