---
summary: "Synology Chat webhook setup and OpenNexus config"
read_when:
  - Setting up Synology Chat with OpenNexus
...
```bash
opennexus plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your OpenNexus gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in OpenNexus.
...
- Pairing approvals work with:
  - `opennexus pairing list synology-chat`
  - `opennexus pairing approve synology-chat <CODE>`
...
```bash
opennexus message send --channel synology-chat --target 123456 --text "Hello from OpenNexus"
opennexus message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
