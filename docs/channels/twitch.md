...
read_when:
  - Setting up Twitch chat integration for OpenNexus
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenNexus connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
opennexus plugins install @opennexus/twitch
```

Local checkout (when running from a git repo):

```bash
opennexus plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENNEXUS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "opennexus", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENNEXUS_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENNEXUS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
