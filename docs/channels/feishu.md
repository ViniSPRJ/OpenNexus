...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OpenNexus to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
opennexus plugins install @opennexus/feishu
```

Local checkout (when running from a git repo):

```bash
opennexus plugins install ./extensions/feishu
```
...
If you just installed OpenNexus, run the wizard:
...
2. Configuring app credentials in OpenNexus
...
1. You already ran `opennexus channels add` for Feishu
2. The gateway is running (`opennexus gateway status`)
...
```bash
opennexus pairing approve feishu <CODE>
```
...
2. Run `opennexus logs --follow` and look for `chat_id`
...
2. Run `opennexus logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `opennexus gateway status`  | Show gateway status           |
| `opennexus gateway install` | Install/start gateway service |
| `opennexus gateway stop`    | Stop gateway service          |
| `opennexus gateway restart` | Restart gateway service       |
| `opennexus logs --follow`   | Tail gateway logs             |
...
4. Check logs: `opennexus logs --follow`
...
5. Ensure the gateway is running: `opennexus gateway status`
6. Check logs: `opennexus logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.opennexus/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.opennexus/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
