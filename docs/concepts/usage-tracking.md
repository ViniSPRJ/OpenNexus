...
- `/usage cost` in chats: local cost summary aggregated from OpenNexus session logs.
- CLI: `opennexus status --usage` prints a full per-provider breakdown.
- CLI: `opennexus channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
