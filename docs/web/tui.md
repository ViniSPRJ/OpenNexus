...
```bash
opennexus tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `opennexus tui --deliver`
...
- Check the Gateway logs: `opennexus logs --follow`.
- Confirm the agent can run: `opennexus status` and `opennexus models status`.
...
- No agents in picker: check `opennexus agents list` and your routing config.
