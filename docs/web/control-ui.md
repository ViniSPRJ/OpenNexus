...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/opennexus`)
...
If the page fails to load, start the Gateway first: `opennexus gateway`.
...
you revoke it with `opennexus devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.opennexus/opennexus.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenNexus
...
OpenNexus **blocks** Control UI connections without device identity.
