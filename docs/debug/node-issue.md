...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenNexus
...
Running OpenNexus via Node with `tsx` fails at startup with:

```
[opennexus] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `opennexus status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch opennexus.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node opennexus.mjs status` works on Node 25.
