---
summary: "Use Venice AI privacy-focused models in OpenNexus"
read_when:
  - You want privacy-focused inference in OpenNexus
...
```bash
opennexus onboard --auth-choice venice-api-key
```
...
```bash
opennexus agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, OpenNexus shows all available Venice models. Pick based on your needs:
...
```bash
opennexus models set venice/claude-opus-45
opennexus models set venice/llama-3.3-70b
```

List all available models:

```bash
opennexus models list | grep venice
```

## Configure via `opennexus configure`

1. Run `opennexus configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenNexus automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
opennexus agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
opennexus agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
opennexus agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
opennexus agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
opennexus agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `opennexus models list` to see currently available models. Some models may be temporarily offline.
