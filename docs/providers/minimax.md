---
summary: "Use MiniMax M2.1 in OpenNexus"
read_when:
  - You want MiniMax models in OpenNexus
...
```bash
opennexus plugins enable minimax-portal-auth  # skip if already loaded.
opennexus gateway restart  # restart if gateway is already running
opennexus onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/opennexus/opennexus/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `opennexus.json`:
...
- Use `opennexus models list` and `opennexus models set minimax/MiniMax-M2.1` to switch.
...
- Running `opennexus configure` and selecting **MiniMax M2.1**, or
