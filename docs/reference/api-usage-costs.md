...
OpenNexus features that can generate provider usage or paid API calls.
...
- `opennexus status --usage` and `opennexus channels list` show provider **usage windows**
...
OpenNexus can pick up credentials from:
...
`opennexus models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
