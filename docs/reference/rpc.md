...
OpenNexus integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenNexus owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenNexus spawns `imsg rpc` as a child process (legacy iMessage integration).
