...
- Jobs persist under `~/.opennexus/cron/` so restarts don’t lose schedules.
...
opennexus cron list
opennexus cron run <job-id>
opennexus cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.opennexus/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `opennexus cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, OpenNexus applies a
...
If `delivery` is omitted for isolated jobs, OpenNexus defaults to `announce`.
...
- Job store: `~/.opennexus/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.opennexus/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
```json5
{
  cron: {
    ...
    store: "~/.opennexus/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, OpenNexus archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `opennexus cron runs` and adjust retention before logs become large
...
opennexus cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
opennexus cron edit <jobId> --agent ops
opennexus cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
opennexus cron run <jobId>
opennexus cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
opennexus cron edit <jobId> \
...
```bash
opennexus cron edit <jobId> --exact
```
...
```bash
opennexus system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`opennexus system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OPENNEXUS_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OpenNexus applies exponential retry backoff for recurring jobs after consecutive errors:
