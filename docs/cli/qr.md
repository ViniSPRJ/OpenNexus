---
summary: "CLI reference for `opennexus qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
opennexus qr
opennexus qr --setup-code-only
opennexus qr --json
opennexus qr --remote
opennexus qr --url wss://gateway.example/ws --token '<token>'
```
