...
Deploy OpenNexus on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/opennexus/opennexus)
...
repository configures everything needed to run OpenNexus:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENNEXUS_STATE_DIR
        value: /data/.opennexus
      - key: OPENNEXUS_WORKSPACE_DIR
        value: /data/workspace
      - key: OPENNEXUS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/opennexus`.
...
If you use the original OpenNexus repository, Render will not auto-deploy your OpenNexus. To update it, run a manual Blueprint sync from the dashboard.
...
For OpenNexus, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OpenNexus host.
