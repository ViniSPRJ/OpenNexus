...
read_when:
  - Designing workspace memory (~/.opennexus/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenNexus integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.opennexus/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.opennexus/workspace` as canonical human-readable memory.
...
```
~/.opennexus/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OpenNexus**, but keep a separable core library.

### Why integrate into OpenNexus?

- OpenNexus already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `opennexus memory recall "…" --k 25 --since 30d`
  - `opennexus memory reflect --since 7d`
...
Pragmatic take for `~/.opennexus/workspace`:
