...
- **gateway host** → `opennexus` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.opennexus/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.opennexus/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `opennexus approvals allowlist ...`).
- `opennexus security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `opennexus doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.opennexus/exec-approvals.json` directly.

CLI: `opennexus approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
