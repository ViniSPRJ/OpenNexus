...
- `gateway`/`node` approvals are controlled by `~/.opennexus/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  OpenNexus prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`opennexus security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `opennexus doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
