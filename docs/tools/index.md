---
summary: "Agent tool surface for OpenNexus (browser, canvas, nodes, message, cron) replacing legacy `opennexus-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `opennexus-*` skills
title: "Tools"
---

# Tools (OpenNexus)

OpenNexus exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `opennexus-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `opennexus.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenNexus logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:opennexus`: all built-in OpenNexus tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`opennexus node run`).
...
OpenNexus tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `opennexus configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OpenNexus-managed browser.
...
- Quick smoke: `opennexus nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `opennexus gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, OpenNexus clamps `tools.sessions.visibility` to `tree`.
