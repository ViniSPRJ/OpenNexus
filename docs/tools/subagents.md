...
- For manual spawns, delivery is resilient:
  - OpenNexus tries direct `agent` delivery first with a stable idempotency key.
...
2. OpenNexus creates or binds a thread to that session target in the active channel.
