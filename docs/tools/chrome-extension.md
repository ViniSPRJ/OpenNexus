---
summary: "Chrome extension: let OpenNexus drive your existing Chrome tab"
read_when:
...
The OpenNexus Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate opennexus-managed Chrome profile.
...
OpenNexus then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenNexus release (npm package) as static files. There is no separate “build” step.

After upgrading OpenNexus:

- Re-run `opennexus browser extension install` to refresh the installed files under your OpenNexus state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

OpenNexus ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `OPENNEXUS_GATEWAY_TOKEN`)
...
- Open the tab you want OpenNexus to control.
...
- `ON`: attached; OpenNexus can drive that tab.
...
Debugging: `opennexus sandbox explain`
...
`opennexus browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `opennexus browser extension install` first to copy the extension to a stable location under your OpenNexus state directory.
...
- **This is not isolated** like the dedicated opennexus-managed profile.
