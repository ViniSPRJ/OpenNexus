...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `opennexus sandbox explain`.
...
Legacy `agent.*` configs are migrated by `opennexus doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=opennexus-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENNEXUS_STATE_DIR:-$HOME/.opennexus}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
