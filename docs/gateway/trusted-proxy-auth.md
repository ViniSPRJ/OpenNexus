...
read_when:
  - Running OpenNexus behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of OpenNexus
...
- You run OpenNexus behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. OpenNexus checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. OpenNexus extracts the user identity from the configured header
...
- OpenNexus can stay on loopback HTTP behind the proxy.
...
If OpenNexus itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://opennexus.example.com
    to: http://opennexus-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
opennexus.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy opennexus:18789 {
...
location / {
    ...
    proxy_pass http://opennexus:18789;
...
`opennexus security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update OpenNexus config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `opennexus security audit` and review findings
