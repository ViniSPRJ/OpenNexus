...
description: "Complete field-by-field reference for ~/.opennexus/opennexus.json"
summary: "Complete reference for every OpenNexus config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.opennexus/opennexus.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OpenNexus uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `opennexus doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["opennexus-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `opennexus plugins install @opennexus/mattermost`.
...
OpenNexus spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@opennexus"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `opennexus.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, OpenNexus auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OpenNexus assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "opennexus-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.opennexus/sandboxes`
...
noVNC observer access uses VNC auth by default and OpenNexus emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `opennexus-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.opennexus/workspace",
        agentDir: "~/.opennexus/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@opennexus"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:opennexus`   | All built-in tools (excludes provider plugins)                                           |
...
OpenNexus uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.opennexus/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `OPENNEXUS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `opennexus onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `opennexus onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `opennexus onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `opennexus onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `opennexus onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `opennexus onboard --auth-choice minimax-api`.
...
- Loaded from `~/.opennexus/extensions`, `<workspace>/.opennexus/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or OPENNEXUS_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `OPENNEXUS_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.opennexus-dev` + port `19001`), `--profile <name>` (uses `~/.opennexus-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.opennexus/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-opennexus-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `OPENNEXUS_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or OPENNEXUS_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__opennexus__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__opennexus__/a2ui/`
...
- Also serves A2UI at `/__opennexus__/a2ui/`.
...
- Hostname defaults to `opennexus`. Override with `OPENNEXUS_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.opennexus/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `opennexus dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.opennexus/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${OPENNEXUS_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.opennexus/credentials/oauth.json`.
...
- Default log file: `/tmp/opennexus/opennexus-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `opennexus doctor --fix` can strip unknown keys).
