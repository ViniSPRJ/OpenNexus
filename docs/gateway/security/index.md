...
## Quick check: `opennexus security audit`
...
OpenNexus is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
OpenNexus assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.opennexus`, including `opennexus.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- OpenNexus can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want OpenNexus, use one VPS/host per user.
...
OpenNexus is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/opennexus/opennexus/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, OpenNexus also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.opennexus/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.opennexus/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.opennexus/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.opennexus/credentials/oauth.json`
...
| -------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full OpenNexus state                               | filesystem perms on `~/.opennexus`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                          | filesystem perms on `~/.opennexus/opennexus.json`                                                   | yes      |
...
`opennexus security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`opennexus security audit` includes `config.insecure_or_dangerous_flags` when any
...
gateway:
  ...
  auth:
    mode: password
    password: ${OPENNEXUS_GATEWAY_PASSWORD}
```
...
- OpenNexus gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from OpenNexus responses.
...
OpenNexus stores session transcripts on disk under `~/.opennexus/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.opennexus` (see the audit section below). If you need
...
OpenNexus can refresh the skills list mid-session:
...
- If you install plugins from npm (`opennexus plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.opennexus/extensions/<pluginId>/` (or `$OPENNEXUS_STATE_DIR/extensions/<pluginId>/`).
  - OpenNexus uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
opennexus pairing list <channel>
opennexus pairing approve <channel> <code>
```
...
By default, OpenNexus routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
OpenNexus has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.opennexus/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.opennexus or your logs.”

## Unsafe external content bypass flags

OpenNexus includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.opennexus/opennexus.json`: `600` (user read/write only)
- `~/.opennexus`: `700` (user only)

`opennexus doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `OPENNEXUS_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__opennexus__/canvas/` and `/__opennexus__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_opennexus-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `OPENNEXUS_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `opennexus doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `OPENNEXUS_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `OPENNEXUS_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), OpenNexus
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. OpenNexus verifies the identity by resolving the
...
- OpenNexus will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.opennexus/` (or `$OPENNEXUS_STATE_DIR/`) may contain secrets or private data:

- `opennexus.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `opennexus status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@opennexus", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.opennexus/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `opennexus` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only OpenNexus clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default OpenNexus limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `opennexus gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `OPENNEXUS_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/opennexus/opennexus-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.opennexus/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `opennexus security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + OpenNexus version
...
Found a vulnerability in OpenNexus? Please report responsibly:

1. Email: [security@opennexus.ai](mailto:security@opennexus.ai)
