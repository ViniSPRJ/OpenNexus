...
你可以在 `~/.opennexus/opennexus.json` 中覆盖此设置：
...
所有日志配置都在 `~/.opennexus/opennexus.json` 的 `logging` 下。
...
- OpenNexus 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `opennexus plugins enable diagnostics-otel` 启用插件。
...
- `opennexus.tokens`（计数器，属性：`opennexus.token`、`opennexus.channel`、`opennexus.provider`、`opennexus.model`）
- `opennexus.cost.usd`（计数器，属性：`opennexus.channel`、`opennexus.provider`、`opennexus.model`）
- `opennexus.run.duration_ms`（直方图，属性：`opennexus.channel`、`opennexus.provider`、`opennexus.model`）
- `opennexus.context.tokens`（直方图，属性：`opennexus.context`、`opennexus.channel`、`opennexus.provider`、`opennexus.model`）

消息流：

- `opennexus.webhook.received`（计数器，属性：`opennexus.channel`、`opennexus.webhook`）
- `opennexus.webhook.error`（计数器，属性：`opennexus.channel`、`opennexus.webhook`）
- `opennexus.webhook.duration_ms`（直方图，属性：`opennexus.channel`、`opennexus.webhook`）
- `opennexus.message.queued`（计数器，属性：`opennexus.channel`、`opennexus.source`）
- `opennexus.message.processed`（计数器，属性：`opennexus.channel`、`opennexus.outcome`）
- `opennexus.message.duration_ms`（直方图，属性：`opennexus.channel`、`opennexus.outcome`）

队列 + 会话：

- `opennexus.queue.lane.enqueue`（计数器，属性：`opennexus.lane`）
- `opennexus.queue.lane.dequeue`（计数器，属性：`opennexus.lane`）
- `opennexus.queue.depth`（直方图，属性：`opennexus.lane` 或 `opennexus.channel=heartbeat`）
- `opennexus.queue.wait_ms`（直方图，属性：`opennexus.lane`）
- `opennexus.session.state`（计数器，属性：`opennexus.state`、`opennexus.reason`）
- `opennexus.session.stuck`（计数器，属性：`opennexus.state`）
- `opennexus.session.stuck_age_ms`（直方图，属性：`opennexus.state`）
- `opennexus.run.attempt`（计数器，属性：`opennexus.attempt`）
...
- `opennexus.webhook.error`
  - `opennexus.channel`、`opennexus.webhook`、`opennexus.chatId`、`opennexus.error`
- `opennexus.message.processed`
  - `opennexus.channel`、`opennexus.outcome`、`opennexus.chatId`、`opennexus.messageId`、`opennexus.sessionKey`、`opennexus.sessionId`、`opennexus.reason`
- `opennexus.session.stuck`
  - `opennexus.state`、`opennexus.ageMs`、`opennexus.queueDepth`、`opennexus.sessionKey`、`opennexus.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `opennexus doctor`。
