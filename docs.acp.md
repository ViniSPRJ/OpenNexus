# OpenNexus ACP Bridge

This document describes how the OpenNexus ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`opennexus acp` exposes an ACP agent over stdio and forwards prompts to a running
OpenNexus Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `opennexus acp` and speaks ACP messages over stdio.
...
`opennexus acp` resolves the Gateway URL and auth from CLI flags or config:
